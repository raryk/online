	...
	connect: function(socket) {
		...
		if (socket && (socket.readyState === 1 || socket.readyState === 0)) {
			...
			try {
				...
				this._map.fire('error', {msg: _('Oops, there is a problem connecting to {productname}: ').replace('{productname}', (typeof brandProductName !== 'undefined' ? brandProductName : 'Perfectum Docs')) + e, cmd: 'socket', kind: 'failed', id: 3});
				return;
			}
		}
		...
		if (textMsg.startsWith('coolserver ')) {
			...
			if (window.indirectSocket) {
				if (window.expectedServerId && window.expectedServerId != this.WSDServer.Id) {
					if (this.IndirectSocketReconnectCount++ >= 3) {
						var msg = errorMessages.clusterconfiguration.replace('{productname}', (typeof brandProductName !== 'undefined' ? brandProductName : 'Perfectum Docs'));
			...
			if (command.errorKind === 'limitreached' && !this.WasShownLimitDialog) {
				...
				textMsg = textMsg.replace('{productname}', (typeof brandProductName !== 'undefined' ?
					brandProductName : 'Perfectum Docs'));
